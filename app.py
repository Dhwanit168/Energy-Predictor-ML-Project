...
def build_gauge(current_val, target_val):
    return go.Figure(
        go.Indicator(
            mode="gauge+number",
            value=current_val,
            number={'suffix': " kWh", 'font': {'size': 36, 'color': bar_color(current_val)}},
            title={'text': "Energy Usage (kWh)", 'font': {'size': 16, 'color': "#555"}},
            gauge={
                'axis': {'range': [0, 120], 'tickwidth': 1, 'tickcolor': "#aaa",
                         'tickvals': [0, 20, 40, 60, 70, 80, 90, 100, 120], 'tickfont': {'size': 11}},
                'bar': {'color': bar_color(current_val), 'thickness': 0.28},
                'bgcolor': "white", 'borderwidth': 1, 'bordercolor': "#ddd",
                'steps': [
                    {'range': [0,  70],  'color': "#d5f5e3"}, {'range': [70, 80],  'color': "#fef9e7"},
                    {'range': [80, 90],  'color': "#fdebd0"}, {'range': [90, 120], 'color': "#fadbd8"},
                ],
                'threshold': {'line': {'color': bar_color(target_val), 'width': 3}, 'thickness': 0.75, 'value': target_val}
            }
        ),
        layout=go.Layout(height=360, autosize=True, margin=dict(t=60, b=20, l=40, r=40), paper_bgcolor="white")
    )

# ── Scroll anchor ──────────────────────────────────────────
st.markdown('<div id="gauge-section"></div>', unsafe_allow_html=True)

# ── Predict Button ─────────────────────────────────────────
predict = st.button("⚡ Predict Energy")

if predict:
    # ── Scroll to gauge section smoothly ──────────────────
    components.html(
        """
        <script>
            setTimeout(function() {
                var el = window.parent.document.getElementById('gauge-section');
                if (el) {
                    el.scrollIntoView({ behavior: 'smooth' });
                }
            }, 50); // scroll almost immediately
        </script>
        """,
        height=0
    )

    # ── Show blank for 2 seconds before gauge ─────────────
    gauge_slot = st.empty()
    with gauge_slot:
        st.markdown("<div style='height:360px;'></div>", unsafe_allow_html=True)
    time.sleep(2)  # 2 seconds blank
    ...
    eased    = np.where(t_vals < 0.5, 4 * t_vals ** 3, 1 - (-2 * t_vals + 2) ** 3 / 2)
    ...
    fig_line.add_trace(go.Scatter(
        x=temps, y=energy_preds, mode='lines+markers', name='Predicted Energy',
        line=dict(color='blue', width=3, shape='spline'), marker=dict(size=6),
        hovertemplate='Temp: %{x}°C<br>Energy: %{y:.2f} kWh'
    ))

    fig_line.add_trace(go.Scatter(
        x=[temp], y=[prediction], mode='markers+text', name='Current Temp',
        marker=dict(color='black', size=14), text=[f"{prediction:.1f} kWh"],
        textposition="top center", hoverinfo="skip"
    ))

    fig_line.update_layout(
        xaxis_title="Temperature (°C)", yaxis_title="Predicted Energy (kWh)",
        yaxis=dict(range=[y_min, y_max]), height=450, autosize=True, hovermode="x unified",
        margin=dict(r=150),
        legend=dict(yanchor="middle", y=0.5, xanchor="left", x=1.02,
                    bgcolor="rgba(255,255,255,0.8)", bordercolor="lightgrey", borderwidth=1)
    )

    st.plotly_chart(fig_line, config={"displayModeBar": False}, key="line_chart")